...
exports.default = new Interaction_1.default('reset', async (client, button, config) => {
    await button.deferReply({ ephemeral: true, fetchReply: true });
    const fetch = await button.editReply({
        embeds: [
            new EmbedBuilder_1.default().default(button.member, config.buttons[button.customId].title, `укажите **пользователя**, которому Вы хотите **сбросить** права в ${button.member.voice.channel.toString()}`)
        ],
        components: new ActionRowBuilder_1.default().menuUser('reset', config.placeholder.user)
    });
    const collector = new discord_js_1.InteractionCollector(client, { time: 30000, message: fetch });
    collector.on('collect', async (interaction) => {
        await interaction.deferUpdate();
        collector.stop();
        return (await Promise.resolve().then(() => __importStar(require('./Collectors/reset')))).default(button, interaction, config);
    });
    collector.on('end', (collected, reasone) => {
        if (reasone === 'time') {
            return button.editReply({
                ...
                components: new ActionRowBuilder_1.default().menuUser('reset', config.placeholder.user, true)
            });
        }
    });
});
