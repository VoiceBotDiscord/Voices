...
exports.default = new Interaction_1.default('kick', async (client, button, config) => {
    await button.deferReply({ ephemeral: true, fetchReply: true });
    const fetch = await button.editReply({
        embeds: [
            new EmbedBuilder_1.default().default(button.member, config.buttons[button.customId].title, `укажите **пользователя**, которого Вы хотите **выгнать** из ${button.member.voice.channel.toString()}`)
        ],
        components: new ActionRowBuilder_1.default().menuUser('kick', config.placeholder.user)
    });
    const collector = new discord_js_1.InteractionCollector(client, { time: 30000, message: fetch });
    collector.on('collect', async (interaction) => {
        await interaction.deferUpdate();
        collector.stop();
        return (await Promise.resolve().then(() => __importStar(require('./Collectors/kick')))).default(client, button, interaction, config);
    });
    collector.on('end', (collected, reasone) => {
        if (reasone === 'time') {
            return button.editReply({
                ...
                components: new ActionRowBuilder_1.default().menuUser('kick', config.placeholder.user, true)
            });
        }
    });
});
